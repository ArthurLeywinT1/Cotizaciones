...
class PanelAcabados extends ConsumerStatefulWidget {
  ...
  // NUEVAS VARIABLES PARA IGUALAR EL CÁLCULO
...
class _PanelAcabadosState extends ConsumerState<PanelAcabados> {
  ...
  void _calcularCostoIndividual(String titulo) {
    final double ancho = double.tryParse(widget.pliegoAnchoController.text) ?? 0.0;
    final double alto = double.tryParse(widget.pliegoAltoController.text) ?? 0.0;
    final double totalPliegos = double.tryParse(widget.totalPliegosController.text) ?? 0.0;
    final double costoCm2 = double.tryParse(widget.controllersCostoCm2[titulo]?.text ?? "0") ?? 0.0;
    final double cantidadImpresion = double.tryParse(widget.cantidadImpresionController.text) ?? 0.0;

    final int lados = (frente[titulo] == true ? 1 : 0) + (vuelta[titulo] == true ? 1 : 0);

    final double area = (ancho * 0.01) * (alto * 0.01);

    double costoCalculado = 0.0;

    // CÁLCULO IDÉNTICO AL DE LAMINADO
    if (widget.isOffset) {
    ...
    if (lados > 0) {
      costoFinal = costoCalculado < costoMinimo ? costoMinimo : costoCalculado;
    }
