...
class _RevistaPageState extends ConsumerState<RevistaPage> {
  ...
  void initState() {
    ...
    if (cotizacion != null) {
      ...
      piezasTotalesInicial =
          widget.piezasOverride ?? cotizacion.cantidadImpresiones;
  ...
  Future<void> _guardarCotizacion() async {
    ...
    if (pliegosList != null) {
      for (var p in pliegosList) {
        if (p is Map) {
          final calculos = p['calculos_impresion'] as Map?;
          final totalP = int.tryParse(
            calculos?['total_pliegos_utilizar']?.toString() ?? '',
          ) ?? 0;
